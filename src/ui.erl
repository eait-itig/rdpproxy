...
label_handler({resize, {W,H}}, Wd = #widget{state = S}) ->
	...
	{ok, _, Image1} = cairerl_nif:draw(Image0, [], [
		...
		#cairo_set_font_size{size = 0.8 * H},
		...
		case Align of
			...
			left -> [
				#cairo_translate{y = H / 2},
				#cairo_scale{y = 0.5},
				#cairo_translate{y = txth},
				#cairo_scale{y = 2.0}];
			right -> [
				#cairo_translate{x = W, y = H / 2},
				#cairo_scale{x = -1.0, y = 0.5},
				#cairo_translate{x = txtw, y = txth},
				#cairo_scale{x = -1.0, y = 2.0}]
		end ++ [
...
-record(textinput_state, {placeholder, text= <<>>, cursor=0, xs=[0.0], base}).
textinput_get(#widget{state = #textinput_state{text = Txt}}) ->
	Txt.
...
textinput_calc_xs(_SoFar, <<>>, _Sz) -> [];
textinput_calc_xs(SoFar, <<Next,Rest/binary>>, Sz = {W, H}) ->
	Image0 = #cairo_image{width = round(W), height = round(H), data = <<>>},
	Text = <<SoFar/binary, Next, 0>>,
	...
	[XAdv | textinput_calc_xs(<<SoFar/binary, Next>>, Rest, Sz)].
...
textinput_handler(#ts_inpevt_mouse{action = down, buttons = [1]}, Wd = #widget{tags = T}) ->
	case lists:member(focus, T) of
		true -> {ok, Wd, []};
		false ->
			textinput_handler(redraw_base, Wd#widget{tags = [focus | T]})
	end;
...
textinput_handler(redraw_text, Wd = #widget{state = S, tags = T, size = {W, H}}) ->
	#textinput_state{text = Text, cursor = N, xs = Xs, base = Image0, placeholder = Placeholder} = S,
	...
	{ok, _, Image1} = case Text of
		...
		_ ->
			cairerl_nif:draw(Image0, [], [
				...
				#cairo_translate{x = 5.0, y = 0.7*H},
				#cairo_show_text{text = <<TextBefore/binary, 0>>},

				#cairo_translate{x = CursorX},

				#cairo_set_line_width{width = 1.5},
				#cairo_move_to{x = 0.0, y = 0.15*H},
				#cairo_line_to{flags = [relative], y = -0.6*H},
				#cairo_stroke{},

				#cairo_show_text{text = <<TextAfter/binary, 0>>}
			])
	end,
	{ok, Wd#widget{orders = [
		#image{image = Image1}
