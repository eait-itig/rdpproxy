...
#define SRCREADPIXEL(_pix, _buf) _pix = (_buf)[0] | ((_buf)[1] << 8) | \
  ((_buf)[2] << 16)
#define DESTNEXTPIXEL(_buf) _buf += 4
#define SRCNEXTPIXEL(_buf) _buf += 3
#define WRITEFGBGIMAGE WriteFgBgImage24to32
#define WRITEFIRSTLINEFGBGIMAGE WriteFirstLineFgBgImage24to32
#define RLEDECOMPRESS RleDecompress24to32
#define RLEEXTRA
#include "bitmap_incl.c"
...
BOOL bitmap_decompress(BYTE* srcData, BYTE* dstData, int width, int height, int size, int srcBpp, int dstBpp)
{
	...
	else if (srcBpp == 24 && dstBpp == 32)
	{
		...
		freerdp_bitmap_flip(TmpBfr, dstData, width * 4, height);
		_aligned_free(TmpBfr);
	}
	else
	{
